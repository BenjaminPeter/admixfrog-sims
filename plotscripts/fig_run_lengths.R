...
sim_raw = read_csv('series/simfrags/A4binsMH/SBasic/DHuman1/C4nocont.xz')
runs = read_csv("series/A4binsMH/SBasic/RBasic/DHuman1/C4nocont.est.xz")
...
sim = sim_raw %>% 
    ...
    group_by(len, coverage, sample_time, bin_size) %>% 
    summarize(n=sum(n)) %>% group_by(coverage, sample_time, bin_size) %>% 
...
ggsave("figures/fraglen1.png", P_runs, width=7, height=3.5)
...
P_sims =  ggplot() + 
    ...
    theme(legend.position='top')
ggsave("figures/fraglen2.png", P_sims, width=8, height=5)
